...
func (c *Client) RefundSale(ctx context.Context, saleID string, a *Amount) (*Refund, error) {
	...
	req, err := c.NewRequest(ctx, "POST", fmt.Sprintf("%s%s", c.APIBase, "/v1/payment/sale/"+saleID+"/refund"), &refundRequest{Amount: a})
	if err != nil {
		return refund, err
	}
...
func (c *Client) GetRefund(ctx context.Coontext, refundID string) (*Refund, error) {
	refund := &Refund{}
