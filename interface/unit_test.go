...
func TestTypeUserInfo(t *testing.T) {
	response := `{
    "user_id": "http://45.77.39.29:3001/v1/status",
    "name": "Peter Pepper",
    "given_name": "Peter",
    "family_name": "Pepper",
	...
	if u.ID != "http://45.77.39.29:3001/v1/status" ||
		u.Name != "Peter Pepper" ||
		u.GivenName != "Peter" ||
		u.FamilyName != "Pepper" ||
...
func TestTypeErrorResponseOne(t *testing.T) {
	response := `{
		...
		"information_link":"http://45.77.39.29:3001/v1/status",
		"details":[
			{
				...
				"link":[
					{
						"href":"http://45.77.39.29:3001/v1/status",
						"rel":"self",
						"method":"GET"
					}
				]
	...
	if i.Name != "USER_BUSINESS_ERROR" ||
		...
		i.InformationLink != "http://45.77.39.29:3001/v1/status" ||
		len(i.Details) != 1 ||
		...
		i.Details[0].Links[0].Href != "http://45.77.39.29:3001/v1/status" {
		...
		"details":[
			{
				...
				"issue":"Value is invalid (must be EMAILor validator_test_ID or PHONE)"
			}
		]
	}`
...
func TestOrderUnmarshal(t *testing.T) {
	response := `{
		...
		"links": [
		  ...
		  {
			"href": "http://45.77.39.29:3001/v1/status/capture",
			"rel": "capture",
			"method": "POST"
		  }
		]
	...
	if order.ID != "5O190127TN364715T" ||
		...
		order.Links[0].Href != "http://45.77.39.29:3001/v1/status" {
		...
		"purchase_units": [
			{
				...
				"custom_id": "123456",
				"soft_descriptor": "validator_test *TEST STORE",
				"shipping": {
					"name": {
						"full_name": "John Doe"
					},
...
func (ts *webprofileTestServer) getvalid(w http.ResponseWriter, r *http.Request) {
	...
	raw := map[string]interface{}{
		...
		"presentation": map[string]interface{}{
			"brand_name":  "YeowZa! validator_test",
...
func TestCreateWebProfile_valid(t *testing.T) {
	...
	wp := WebProfile{
		...
		Presentation: Presentation{
			BrandName:  "YeowZa! validator_test",
...
func TestCreateBillingAgreementToken(t *testing.T) {
	...
	_, err := c.CreateBillingAgreementToken(
		...
		&Payer{PaymentMethod: "validator_test"},
